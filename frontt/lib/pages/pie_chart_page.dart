        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                // Dropdown untuk bulan (pakai nama bulan)
                DropdownButton<int>(
                  value: selectedMonth,
                  onChanged: (val) {
                    if (val != null) {
                  ...
                  items: List.generate(12, (i) => i + 1)
                      .map((m) => DropdownMenuItem(
                            value: m,
                            child: Text(DateFormat('MMMM', 'id').format(DateTime(0, m))),
                          ))
                      .toList(),
                ),
...
LineChartData _buildLineChartData(String type) {
  ...
  final Map<DateTime, String> labelData = {}; // Simpan label kategori terakhir

  for (var t in filteredTransactions.where((tx) => tx.type == type)) {
    final date = DateFormat('EEE, dd MMM yyyy HH:mm:ss', 'en_US').parse(t.date);
    final day = DateTime(date.year, date.month, date.day);
    dailyData[day] = (dailyData[day] ?? 0) + t.amount;
    labelData[day] = t.categoryName; // Simpan kategori terakhir pada tanggal tsb
  }

  if (dailyData.isEmpty) {
    return LineChartData(); // Hindari crash saat data kosong
  }
  ...
  final lineColor = isPemasukan ? const Color.fromARGB(255, 44, 104, 182) : const Color.fromARGB(255, 54, 193, 244);
      ...
      leftTitles: AxisTitles(
        sideTitles: SideTitles(
          ...
          getTitlesWidget: (value, meta) => Text(
            NumberFormat.decimalPattern('id').format(value), // format angka
      ...
      touchTooltipData: LineTouchTooltipData(
        ...
        getTooltipItems: (touchedSpots) {
          return touchedSpots.map((spot) {
            ...
            final formattedValue = NumberFormat.decimalPattern('id').format(spot.y);
